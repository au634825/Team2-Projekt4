...
int lightIntensity(int input){
    pwm_mosfet_period(500000);
    pwm_mosfet_duty(0);
    pwm_mosfet_enable(1); //rød led
...
//        }
        ...
        // 25%
        if (input == 25 ){
            pwm_mosfet_duty(femogtyve_procent); //skal tweakes se beregninger timebox 6
            printf(">> indtastet er: %d duty bliver: %d\n\n"
        ...
        // 50%
        if (input == 50 ){
            pwm_mosfet_duty(halvtreds_procent); //skal tweakes se beregninger timebox 6
            printf(">> indtastet er: %d duty bliver: %d\n\n"
        ...
        // 75%
        if (input == 75 ){
            pwm_mosfet_duty(femoghalvfjers_procent); //skal tweakes se beregninger timebox 6
            printf(">> indtastet er: %d duty bliver: %d\n\n"
        ...
        // 100%
        if (input == 100 ){
            pwm_mosfet_duty(ethundrede_procent);
            printf(">> indtastet er: %d duty bliver: %d\n\n"
